...
data {
    ...
    vector [nEV + 2] obs_mAVprojs [nsne];
    matrix [nEV + 2, nEV + 2] obs_Lmats [nsne];
}
...
model {
    ...
    vector [nEV+2] true_mAVprojs [nsne];

    for (i in 1:nsne) {
        xstar_by_SN[i] <- xstar * redcoeff[i];
        Rx_by_SN[i] <- Rx * redcoeff[i];
        tau_AV_by_SN[i] <- tau_AV * redcoeff[i];
        true_mAVprojs[i][1] <- dM[i];
        true_mAVprojs[i][2] <- true_AV[i];
        for (j in 1:nEV) {
            true_mAVprojs[i][2+j] <- true_x[i][j];
        }
    }

    for (i in 1:nsne) {
        obs_mAVprojs[i] ~ multi_normal_cholesky(true_mAVprojs[i], obs_Lmats[i]);
        true_x[i] ~ normal(xstar_by_SN[i], Rx_by_SN[i]);
...
stan_data = dict(nsne = 0, nredcoeff = 2, nobs = 0, nEV = 14,
                 ...
                 lowind = [], highind = [], projs_0 = projs, norms_0 = smoothed_norms,
...
for i in range(SN_data["nsne"]):
    ...
    if len(SN_data["SN_observations"][i]["fluxes"]) > 4:
        ...
        phaseinds = where(filts_are_okay_mask*(phases > -10)*(phases < 30))

        filts = unique(SN_data["SN_observations"][i]["filts"][inds])
        if ((len(filts) > 2) and (sum(SNRs[inds] > 8) > 4) and (len(SNRs[phaseinds]) > 16)) or has_IFC:
...
def pullfn(P, thisdata):
    obs_fluxes, obs_dfluxes, mean_eval, color_law, EVs = thisdata[0]
    the_mod = (dot(EVs, P[2:]) + mean_eval) * 10.**(-0.4*(color_law * P[1] + P[0]))
...
stan_data["obs_Lmats"] = []
stan_data["obs_mAVprojs"] = []
for i in range(stan_data["nsne"]):
...
stan_data["obs_mAVprojs"] = array(stan_data["obs_mAVprojs"])
stan_data["obs_Lmats"] = array(stan_data["obs_Lmats"])
